...
sts view_mp3(char *mp3) {
    FILE *fptr;
    if (open_mp3_view(&fptr, mp3) == e_failure) return e_failure; // opening file

    fread(buffer, 3, 1, fptr); // reading first three bytes to check whether it ID3 or not
...
sts read_tag_view(FILE *fptr) {
    while (!feof(fptr)) { // if EOF encountered get out of the loop
        if (fread(buffer, 4, 1, fptr) != 1) break;

        for (int j = 0; j < 6; j++) {
            if (strncmp(buffer, TAG[j], 4) == 0) {
                int size = get_tag_size_view(fptr); // getting tag size to read its content to print
                fseek(fptr, 2, SEEK_CUR);  // skip flags (2 bytes)
                printf("%s : %10s",TAG[j],tag_name[j]);
                fread(buffer, size, 1, fptr);
                print_tag_content(buffer, size); // calling this fun. to print tag content byte by byte
            }
        }
    }
    return e_success;
...
int get_tag_size_view(FILE *fptr) {
    ...
    int size = (size_buf[0] & 0x7F) << 21|(size_buf[1] & 0x7F) << 14|(size_buf[2] & 0x7F) << 7|(size_buf[3] & 0x7F); // Big endian to little endian conversion
    return size;
}
